...
SleekVim targets *only* the latest
...
[discussion in #1740](https://github.com/nvim-lua/SleekVim/issues/1740)
for the tradeoffs between the two approaches.

> [!NOTE]
> Your repo's URL will be something like this:
> `https://github.com/<your_github_username>/SleekVim.git`

You likely want to remove `nvim-pack-lock.json` from your repo's `.gitignore`
file too - it's ignored in the SleekVim repo to make maintenance easier, but
...
```sh
git clone https://github.com/nvim-lua/SleekVim.git "${XDG_CONFIG_HOME:-$HOME/.config}"/nvim
```
...
```
git clone https://github.com/nvim-lua/SleekVim.git "%localappdata%\nvim"
```

If you're using `powershell.exe`

```
git clone https://github.com/nvim-lua/SleekVim.git "${env:LOCALAPPDATA}\nvim"
```
...
* Can I keep my existing configuration in parallel to SleekVim?
  * Yes! You can use [NVIM_APPNAME](https://neovim.io/doc/user/starting.html#%24NVIM_APPNAME)`=nvim-NAME`
    to maintain multiple configurations. For example, you can install the SleekVim
    configuration in `~/.config/nvim-SleekVim` and create an alias:
    ```
    alias nvim-SleekVim='NVIM_APPNAME="nvim-SleekVim" nvim'
    ```
    When you run Neovim using `nvim-SleekVim` alias it will use the alternative
    config directory and the matching local directory
    `~/.local/share/nvim-SleekVim`. You can apply this approach to any Neovim
    distribution that you would like to try out.
* What if I want to "uninstall" this configuration:
  * Remove your config directory and local data directory (for example,
    `~/.config/nvim` and `~/.local/share/nvim`).
* Why is the SleekVim `init.lua` a single file? Wouldn't it make sense to split it into multiple files?
  * The main purpose of SleekVim is to serve as a teaching tool and a reference
    configuration that someone can easily use to `git clone` as a basis for their own.
    As you progress in learning Neovim and Lua, you might consider splitting `init.lua`
    into smaller parts. A fork of SleekVim that does this while maintaining the
    same functionality is available here:
    * [SleekVim-modular.nvim](https://github.com/dam9000/SleekVim-modular.nvim)
  * Discussions on this topic can be found here:
    * [Restructure the configuration](https://github.com/nvim-lua/SleekVim/issues/218)
    * [Reorganize init.lua into a multi-file setup](https://github.com/nvim-lua/SleekVim/pull/473)

### Install Recipes

Below you can find OS specific install instructions for Neovim and dependencies.

After installing all the dependencies continue with the [Install SleekVim](#install-SleekVim) step.

#### Windows Installation

<details><summary>Windows with Microsoft C++ Build Tools and CMake</summary>
SleekVim's default config is make-only for `telescope-fzf-native.nvim`.
...
[here](https://github.com/nvim-lua/SleekVim/issues/1583).
